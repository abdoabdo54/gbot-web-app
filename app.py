...
@app.route('/api/test-smtp-advanced', methods=['POST'])
def test_smtp_advanced():
    """Advanced SMTP testing endpoint with comprehensive error handling and multiple test modes"""
    try:
        data = request.get_json()
        ...
        test_mode = data.get('test_mode', 'connection_only')
        ...
        if test_mode in ['send_emails', 'batch_test'] and not recipients:
            return jsonify({
                'success': False,
                'error': 'Recipients are required for email sending tests'
            })
        
        app.logger.info(f"Starting advanced SMTP test - Mode: {test_mode}, Server: {server}:{port}, Email: {email}")
        ...
        try:
            ...
            if security == 'ssl':
                ...
                smtp_server = smtplib.SMTP_SSL(server, port, timeout=timeout)
                app.logger.info(f"Created SSL connection to {server}:{port}")
            else:
                # Regular connection with optional STARTTLS
                smtp_server = smtplib.SMTP(server, port, timeout=timeout)
                app.logger.info(f"Created SMTP connection to {server}:{port}")
                
                if security == 'tls':
                    # Enable debug output for troubleshooting
                    smtp_server.set_debuglevel(1)
                    smtp_server.starttls()
                    app.logger.info("STARTTLS enabled")
            
            # Authenticate
            smtp_server.login(email, password)
            app.logger.info(f"Successfully authenticated with {email}")
            
            # Determine what to do based on test mode
            if test_mode == 'connection_only':
                # Just test connection and authentication
                smtp_server.quit()
                return jsonify({
                    'success': True,
                    'message': f'Connection test successful for {email} on {server}:{port}',
                    'details': {
                        'server': server,
                        'port': port,
                        'security': security,
                        'email': email,
                        'test_type': 'connection_only'
                    }
                })
            
            elif test_mode in ['send_emails', 'batch_test']:
                # Send actual test emails
                sent_count = 0
                failed_recipients = []
                
                # Prepare email content
                subject = email_data.get('subject', 'SMTP Test Email')
                body = email_data.get('body', 'This is a test email from the SMTP tester.')
                ...
                for recipient in recipients:
                    try:
                        ...
                        app.logger.error(f"Failed to send email to {recipient}: {recipient_error}")
                
                smtp_server.quit()
                
                # Prepare response
                if sent_count > 0:
                    message = f'Successfully sent {sent_count} emails from {email}'
                    if failed_recipients:
                        message += f', {len(failed_recipients)} failed'
                    ...
                    return jsonify({
                        'success': False,
                        'error': f'Failed to send any emails. All {len(failed_recipients)} recipients failed.',
                        'details': {
                            'failed_recipients': failed_recipients
                        }
                    })
                ...
                return jsonify({
                    'success': False,
                    'error': f'Unknown test mode: {test_mode}'
                })
                
        except smtplib.SMTPAuthenticationError as auth_error:
            error_msg = f"Authentication failed for {email}: {str(auth_error)}"
            ...
            error_msg = f"Server disconnected unexpectedly: {str(disconnect_error)}"
            app.logger.error(error_msg)
            return jsonify({
                'success': False,
                'error': error_msg,
            ...
            error_msg = f"Connection timeout to {server}:{port} (timeout: {timeout}s)"
            app.logger.error(error_msg)
            return jsonify({
                'success': False,
                'error': error_msg,
                ...
                'suggestions': [
                    ...
                    'Check if server is blocking connections'
                ]
            })
            
        except socket.gaierror as dns_error:
            error_msg = f"DNS resolution failed for {server}: {str(dns_error)}"
            app.logger.error(error_msg)
            return jsonify({
                'success': False,
                'error': error_msg,
            ...
            error_msg = f"SSL/TLS error: {str(ssl_error)}"
            app.logger.error(error_msg)
            return jsonify({
                'success': False,
                'error': error_msg,
                'error_type': 'ssl',
                'suggestions': [
                    'Try different security mode (TLS vs SSL)',
                    'Check if server supports the selected security protocol',
            ...
            app.logger.error(f"Unexpected SMTP error: {smtp_error}")
            return jsonify({
                'success': False,
                'error': error_msg,
                'error_type': 'general',
        ...
        error_msg = f"System error during SMTP test: {str(e)}"
        app.logger.error(f"System error in test_smtp_advanced: {e}")
        return jsonify({
            'success': False,
            'error': error_msg,
            'error_type': 'system'
