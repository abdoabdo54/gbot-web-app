...
def api_create_random_users():
    """Create random users in Google Workspace"""
    try:
        data = request.get_json()
        account_name = data.get('account_name')
        ...
        if not account.is_authenticated:
            return jsonify({'success': False, 'error': 'Account not authenticated'})
        
        # Initialize Google API service
        google_api = GoogleAPI(account.client_id, account.client_secret, account.refresh_token)
        service = google_api.service
        
        if not service:
            return jsonify({'success': False, 'error': 'Failed to initialize Google API service'})
        ...
        first_names = ['John', 'Jane', 'Michael', 'Sarah', 'David', 'Emily', 'Robert', 'Jessica', 'William', 'Ashley',
                      'James', 'Amanda', 'Daniel', 'Jennifer', 'Christopher', 'Stephanie', 'Matthew', 'Melissa',
                      'Anthony', 'Nicole', 'Mark', 'Elizabeth', 'Donald', 'Helen', 'Steven', 'Deborah', 'Paul',
                      'Rachel', 'Andrew', 'Carolyn', 'Joshua', 'Janet', 'Kenneth', 'Catherine', 'Kevin', 'Frances']
        ...
        for i in range(count):
            try:
                ...
                if result.get('primaryEmail'):
                    ...
                    app.logger.info(f"Created random user: {email}")
                else:
                    failed_users.append({'email': email, 'error': 'User creation failed'})
                    
            except Exception as e:
                failed_users.append({'email': f"{username}@{domain}", 'error': str(e)})
                app.logger.error(f"Error creating random user {i+1}: {e}")
        ...
        app.logger.error(f"Error creating random users: {e}")
        return jsonify({'success': False, 'error': str(e)})
...
def api_test_smtp():
    ...
    try:
        ...
        if not server:
            return jsonify({'success': False, 'error': 'No SMTP server provided'})
        ...
        for cred in credentials:
            ...
            try:
                ...
                msg['From'] = email
                msg['To'] = recipient
                msg['Subject'] = 'SMTP Test - GBot Web App'
                
                body = f"""
                This is a test email from GBot Web Application.
                ...
                app.logger.info(f"SMTP test successful for {email}")
                ...
                failed += 1
                app.logger.warning(f"SMTP auth failed for {email}")
                
            except smtplib.SMTPConnectError:
                results.append({
                    'email': email,
                    'success': False,
                    'error': 'Connection failed - Check server/port'
                })
                failed += 1
                app.logger.warning(f"SMTP connection failed for {email}")
                ...
                failed += 1
                app.logger.warning(f"SMTP error for {email}: {e}")
                ...
                failed += 1
                app.logger.error(f"Unexpected SMTP error for {email}: {e}")
        ...
        app.logger.error(f"Error in SMTP test: {e}")
        return jsonify({'success': False, 'error': str(e)})
