...
@app.route('/api/retrieve-remote-accounts', methods=['POST'])
@login_required
def api_retrieve_remote_accounts():
    """Retrieve accounts from remote server"""
    try:
        # Check if user is mailer role (not allowed to retrieve accounts)
        if session.get('role') == 'mailer':
            return jsonify({'success': False, 'error': 'Mailer users cannot retrieve accounts'})
        
        # Retrieve accounts from server
        accounts = account_manager.retrieve_accounts_from_server()
        
        if not accounts:
            return jsonify({'success': False, 'error': 'No accounts found on server or connection failed'})
        ...
        return jsonify({
            'success': True,
            'message': f'Retrieved {len(accounts)} accounts from server',
            'accounts': accounts,
            'count': len(accounts)
        })
        
    except Exception as e:
        logging.error(f"Retrieve remote accounts error: {e}")
        return jsonify({'success': False, 'error': str(e)})

@app.route('/api/login-selected-account', methods=['POST'])
@login_required
def api_login_selected_account():
    """Login to a selected account using Chrome automation"""
    try:
        data = request.get_json()
        account_name = data.get('account_name')
        oauth_url = data.get('oauth_url')
        
        if not account_name or not oauth_url:
            return jsonify({'success': False, 'error': 'Account name and OAuth URL required'})
        
        # Get retrieved accounts from session
        retrieved_accounts = session.get('retrieved_accounts', [])
        
        # Find the account in retrieved accounts
        account_data = None
        for acc in retrieved_accounts:
            if acc['account'] == account_name:
                account_data = acc
                break
        
        if not account_data:
            return jsonify({'success': False, 'error': f'Account {account_name} not found in retrieved accounts'})
        
        # Perform login using Chrome automation
        result = account_manager.login_to_google_account(
        ...
        if result['success']:
            # Extract authorization code from final URL if present
        ...
        logging.error(f"Login selected account error: {e}")
        return jsonify({'success': False, 'error': str(e)})

@app.route('/api/login-multiple-accounts', methods=['POST'])
@login_required
def api_login_multiple_accounts():
    """Login to multiple accounts using Chrome automation with concurrency control"""
    try:
        data = request.get_json()
        ...
        if not account_names or not oauth_url:
            return jsonify({'success': False, 'error': 'Account names and OAuth URL required'})
        ...
        accounts_to_login = []
        for acc in retrieved_accounts:
            if acc['account'] in account_names:
                accounts_to_login.append(acc)
        
        if not accounts_to_login:
            return jsonify({'success': False, 'error': 'No valid accounts found for login'})
        
        # Add accounts to login queue
        queue_ids = []
        for acc in accounts_to_login:
            queue_id = account_manager.add_login_to_queue(
                acc['account'],
                acc['password'],
                oauth_url
            )
        ...
        queue_thread = threading.Thread(
            target=account_manager.process_login_queue,
            args=(max_concurrent,),
            daemon=True
        )
        queue_thread.start()
        ...
        return jsonify({
            'success': True,
            'message': f'Added {len(accounts_to_login)} accounts to login queue',
            'queue_ids': queue_ids,
            'max_concurrent': max_concurrent
        })
        
    except Exception as e:
        logging.error(f"Login multiple accounts error: {e}")
        return jsonify({'success': False, 'error': str(e)})

@app.route('/api/get-login-status', methods=['POST'])
@login_required
def api_get_login_status():
    """Get status of login queue and completed logins"""
    try:
        data = request.get_json()
        queue_ids = data.get('queue_ids', [])
        
        # This would need to be implemented with proper queue tracking
        # For now, return a simple status
        return jsonify({
            'success': True,
            'status': 'Processing',
            'message': 'Login queue is being processed'
        })
        
    except Exception as e:
        logging.error(f"Get login status error: {e}")
        return jsonify({'success': False, 'error': str(e)})

@app.route('/api/clear-retrieved-accounts', methods=['POST'])
@login_required
def api_clear_retrieved_accounts():
    """Clear retrieved accounts from session"""
    try:
        if 'retrieved_accounts' in session:
            del session['retrieved_accounts']
        
        return jsonify({
            'success': True,
            'message': 'Retrieved accounts cleared from session'
        })
        
    except Exception as e:
        logging.error(f"Clear retrieved accounts error: {e}")
        return jsonify({'success': False, 'error': str(e)})
